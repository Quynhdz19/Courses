...
const CourseUsersManagementPage = () => {
    ...
    const handleSelectAll = () => {
        if (activeTab === 'users') {
            setSelectedUsers(usersCourse.length === selectedUsers.length ? [] : usersCourse.map(user => user._id))
        } else if (activeTab === 'add-users') {
            setSelectedUsers(users.length === selectedUsers.length ? [] : users.map(user => user._id))
        }
    }

    const isHeaderCheckboxChecked = activeTab === 'users'
    ...
    return (
        <CContainer>
            <CNav variant="tabs" className="mb-4">
                ...
                <CNavItem>
                    <CNavLink active={activeTab === 'export-exl'} onClick={() => handleTabChange('export-exl')}>
                        Export Excel
                    </CNavLink>
                </CNavItem>
            </CNav>
                    ...
                    <div className="d-flex justify-content-center mt-3">
                        <CPagination aria-label="Page navigation example">
                            ...
                            {[...Array(totalPages)].map((_, i) => (
                                <CPaginationItem key={i + 1} active={currentPage === i + 1} onClick={() => handlePageChange(i + 1)}>{i + 1}</CPaginationItem>
                            ))}
                ...
                <CCard className="py-5">
                    <CCardBody className="text-center">
                        <div>
                            ...
                            <CButton color="primary" className="mt-3" onClick={handleExportExcel}>Export</CButton>
                        </div>
                    </CCardBody>
                </CCard>
            )}
